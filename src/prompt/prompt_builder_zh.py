"""
Prompt Builder (Chinese Version) - 組裝最終的 System Prompt
...
class PromptBuilder:
    ...
    def build_system_prompt(behavior_specs: dict, character_info: dict, conversation_history: list = None, rag_context: dict = None) -> str:
        ...
        參數：
            ...
            conversation_history: list，當前未摘要的對話歷史（可選，組裝進 System Prompt）
                [{role: str, text: str}, ...]
            rag_context: dict，RAG 檢索結果（可選，包含最相關背景、範例、摘要）
...
- **核心特徵**: [{tag_str}]"""
        ...
        if conversation_history:
            ...
            for msg in conversation_history:
                role_label = "用戶" if msg.get("role") == "user" else "角色"
                conversation_context_str += f"\n【{role_label}】\n{msg.get('text', '')}"

        # 12. 組裝最終 system prompt
        prompt = f"""# 系統角色扮演指令
